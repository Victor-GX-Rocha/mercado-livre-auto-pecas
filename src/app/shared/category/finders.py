...
class IDFinderByPath:
    ...
    def find(self, category_path: str, access_token: str) -> CategoryFinderResponse: 
        """
        Find a category ID based on his his path. 
        Args:
        ...
        for level, category_name in enumerate(category_names):
            atual_category = self._find_category_level_name(category_name, categories)
            
            if not atual_category:
                return CategoryFinderResponse(error=f"Nível '{category_name}' não encontrado.")
            
            if level < len(category_names) - 1:
                category_data_response = self.category_requests.get_category_data(atual_category["id"], access_token)
        ...
        return CategoryFinderResponse(success=True, result=atual_category["id"])
