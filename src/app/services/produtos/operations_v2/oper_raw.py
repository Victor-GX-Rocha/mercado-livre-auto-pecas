...
class BaseEdit(ProdutosOperationProtocol):
    def __init__(
        self, 
        ...
        self.edition_data: dict[str, str] = None
    ...
    def action(self, line: Product, access_token: str) -> None:
        ...
        if not response.success:
            log.dev.exception(str(response))
            self.repo.update.log_error(id=line.id, cod_erro=89, log_erro=response.error)
        
        item_status: str = response.data.get("status")
        
        if item_status != self.oper_status:
            self.repo.update.log_error(id=line.id, cod_erro=89, log_erro=response.error)
        
        log.user.info(f"[DB-ID: {line.id} | Cod: {line.identfiers.cod_produto}] Produto {self.operation_name} com sucesso!")
        line.produto_status = item_status
        self.repo.update.pause_success(line.id, line.produto_status)


class Pause(BaseEdit):
    ...
    def execute(self, lines: list[Product], token: AuthResponse) -> None:
        ...
        self.edition_data: dict[str, str] = {"status": "paused"}
        self.operation_name: str = "pausado"
    ...
    def pause(self, line: Product, access_token: str) -> None:
        ...
        self.action(line, access_token)


class Activation(BaseEdit):
    def execute(self, lines: list[Product], token: AuthResponse) -> None:
        """  
        When I try to edit a product, I need to verify if his status is the same of the editatiton that I'm doing!
        There is possible that the request works, but the status is not changed.
        ...
        self.edition_data: dict[str, str] = {"status": "active"}
        self.operation_name: str = "ativado"
...
class Deletation(BaseEdit):...



class PauseOld(ProdutosOperationProtocol, BaseEditValidation):
    def __init__(
        self, 
        repo: ProdutosRepository, 
        items_requests: ItemsRequests
...
class Activation(ProdutosOperationProtocol):
    def __init__(
        self, 
        repo: ProdutosRepository, 
        items_requests: ItemsRequests
...
class JustSleep(ProdutosOperationProtocol):
...
class InvalidOperation(ProdutosOperationProtocol):
    def __init__(self, repo: ProdutosRepository):
        self.repo = repo
...
class Edition(ProdutosOperationProtocol):
    ...
    def _register_results(self, line: Product) -> None:
        ...
        """
        if self.fails:
            self.repo.update.log_error(id=line.id, cod_erro=89, log_erro=self.fails)
